...
public class DownloadManager implements DTListener, PeerUpdateListener, 
    ...
    private long unnecessaryPieces;
    ...
    public DownloadManager(TorrentFile torrent, final byte[] clientID) {
    	this(torrent, clientID, null);
    }
    
    public DownloadManager(TorrentFile torrent, final byte[] clientID, List<String> lightList) {
        ...
        this.downloadRate = 0;
        this.unnecessaryPieces = 0;
        this.downloadFinished = false;
        this.interrupt = false;
        
        // initializing black list
        this.blacklist = new boolean[this.nbOfFiles];        
        for(int i=0;i<blacklist.length && lightList!=null;i++)
        	blacklist[i] = !(lightList.contains(this.torrent.name.get(i)));
        ...
        for (int i = 0; i < this.nbPieces; i++) {
            ...
            do {
           	//System.out.println("passing");
                ...
                if(!blacklist[file])flag = true;
                // the remaining data in the piece is more than the remaining data in the file
                if ((fileoffset + this.torrent.pieceLength - pieceoffset >=
                    (Integer) (torrent.length.get(file)) &&
                    i != this.nbPieces - 1) || 
                    (i == this.nbPieces -1 && fileoffset + (this.length % this.torrent.pieceLength) - pieceoffset >= 
                    	(Integer) (torrent.length.get(file)))) {
                    pieceoffset += ((Integer) (torrent.length.get(file))).
                            intValue() - fileoffset;
                    //System.out.println(this.torrent.name.get(file));
                    file++;
                    //System.out.println("(i,f)=" + i + " " + file + " " + pieceoffset + 
                    //		" " + (this.length % this.torrent.pieceLength) + " " + this.nbPieces);
                    fileoffset = 0;
                    if ((i != this.nbPieces -1 && pieceoffset == this.torrent.pieceLength) || 
                    		(i == (this.nbPieces - 1) && pieceoffset == (this.length % this.torrent.pieceLength))){
                        break;
                    }
                } else {
            ...
            if(!flag)this.unnecessaryPieces ++;
            if (!flag || this.testComplete(i)) {
    ...
    public synchronized void savePiece(int piece) {
        ...
        //System.out.println(this.pieceList[piece].getFileAndOffset().keySet());
        for (Iterator<Integer> it = this.pieceList[piece].getFileAndOffset().keySet().
                           iterator(); it.hasNext(); ) {
            try {
                Integer file = (Integer) (it.next());
                int offset = ((Integer) (this.pieceList[piece].
                        getFileAndOffset().
                        get(file))).intValue();
                int remaining = ((Integer)this.torrent.length.get(file.intValue())).
                                intValue()
                                - offset;
                int toWrite = (remaining < data.length - writtenSoFar) ? remaining : 
                	data.length - writtenSoFar;
                if(!blacklist[file.intValue()]){
                	this.output_files[file.intValue()].seek(offset);
                	this.output_files[file.intValue()].write(data, writtenSoFar, toWrite);
                }
...
//                                       "ko/s");
    ...
    public synchronized byte[] getPieceFromFiles(int piece) {
        ...
        for (Iterator<Integer> it = this.pieceList[piece].getFileAndOffset().keySet().
            ...
            try {
                ...
                int toWrite = (remaining < data.length - writtenSoFar) ? remaining : 
                	data.length - writtenSoFar;
                //System.out.println("remaininig, toWrite, offset" + remaining + " " + toWrite + " " + offset );
                if(!blacklist[file.intValue()]){
                	this.output_files[file.intValue()].seek(offset);
                	this.output_files[file.intValue()].read(data, writtenSoFar, toWrite);
                }
                else{
                	for(int i=0;i<toWrite;i++)data[writtenSoFar + i] = 0;
                }
    ...
    /**
     * How much in percents of the work is done.
     * @return How much of the work is already done.
     */
    public double getPercentsCompleted(){
//    	System.out.println("u, c, all" + this.unnecessaryPieces + " " + this.isComplete.cardinality() + 
//    			" " + this.nbPieces);
    	if(this.nbOfFiles == this.unnecessaryPieces)return 100.0;
    	return ((100.0) * (this.isComplete.cardinality() - this.unnecessaryPieces)) / (this.nbPieces - this.unnecessaryPieces);
    }
    
    /**
     * The speed at which the torrents was downloaded in the last few seconds.
     * @return The download Rate in kb/s.
     */
    ...
    /**
     * Returns if the download has finished and the torrents is only seeding.
     * @return
     */
    ...
    /**
     * Sets that the work should cease as soon as possible.
     */
     ...
     * @return True iff the work should be stopped.
     */
