 ...
 * to which peer.
 */
public class DownloadManager_backup implements DTListener, PeerUpdateListener,
     ...
     * @param clientID byte[]
     */
    public DownloadManager_backup(TorrentFile torrent, final byte[] clientID) {
        ...
        for (Iterator<Integer> it = this.pieceList[piece].getFileAndOffset().keySet().
            ...
            try {
                ...
                int remaining = ((Integer)this.torrent.length.get(file.intValue())).
                                intValue()
                                - offset;
                int toWrite = (remaining < data.length - offset - writtenSoFar) ? remaining : 
                	data.length - offset - writtenSoFar;
                this.output_files[file.intValue()].seek(offset);
                this.output_files[file.intValue()].write(data,
                        offset + writtenSoFar, toWrite);
